--[[
    FarmV2 — ui/gui.lua
    GUI builder using LinoriaLib.
    Clean tabbed layout with live status indicators.
]]
...
-- LinoriaLib references
local Library      = nil
local ThemeManager = nil
local SaveManager  = nil

-------------------------------------------------
-- Fruit list from game data
-------------------------------------------------
...
function GUI.build(state)
    ...
    local Window = Library:CreateWindow({
        Title  = "FarmV2 | Build A Farm Factory",
        Center = true,
        AutoShow = true,
    })
    ...
    -- Live stats label (updated in a loop below)
    ...
    InfoBox:AddLabel("Each module runs independently.")
    InfoBox:AddLabel("No more crashes from multitasking.")
    ...
    HarvestBox:AddSlider("BatchSize", {
        Text = "Clicks per Frame",
        Default = Config.Farmer.batchSize or 5,
        Min = 1,
        Max = 20,
        Rounding = 0,
        Callback = function(v) Config.Farmer.batchSize = v end,
    })

    HarvestBox:AddSlider("ClusterRadius", {
        Text = "Cluster Radius (studs)",
        Default = Config.Farmer.clusterRadius or 20,
        Min = 5,
        Max = 100,
        Rounding = 0,
        Callback = function(v) Config.Farmer.clusterRadius = v end,
    })

    HarvestBox:AddSlider("MaxPerCycle", {
        Text = "Max Tiles per Cycle",
        Default = Config.Farmer.maxPerCycle or 9999,
        Min = 10,
        Max = 9999,
        Rounding = 0,
        Callback = function(v) Config.Farmer.maxPerCycle = v end,
    })
    ...
    TpBox:AddDropdown("TpMode", {
        Values = { "instant", "safe" },
        Default = Config.Farmer.tpMode or "instant",
        Text = "TP Mode",
        Callback = function(v) Config.Farmer.tpMode = v end,
    })

    TpBox:AddSlider("SafeTpStep", {
        Text = "Safe TP Step (studs)",
        Default = Config.Farmer.safeTpStep or 100,
        Min = 10,
        Max = 500,
        Rounding = 0,
        Callback = function(v) Config.Farmer.safeTpStep = v end,
    })
    ...
    SniperCtrl:AddButton({
        Text = "▶ Start Sniper",
        Func = function() Sniper.start() end,
    })

    SniperCtrl:AddButton({
        Text = "⏹ Stop Sniper",
        Func = function() Sniper.stop() end,
    })
    ...
    SellBox:AddSlider("SellInterval", {
        ...
        Max = 5,
        Rounding = 2,
        Callback = function(v) Config.AutoSell.sellInterval = v end,
    })
    ...
    GenBox:AddToggle("AntiAFK", {
        ...
        Callback = function(v) AntiAFK.setAntiAFK(v) end,
    })

    GenBox:AddToggle("NoClip", {
        Text = "👻 NoClip",
        Default = Config.AntiAFK.noClip ~= false,
        Callback = function(v) AntiAFK.setNoClip(v) end,
    })
    ...
    NetBox:AddLabel("Rate limits per remote (calls/sec):")
    NetBox:AddSlider("ClickRate", {
        ...
        Max = 60,
        Rounding = 0,
        Callback = function(v) Network.setLimit("ClickPlant", v, v) end,
    })
    NetBox:AddSlider("RollRate", {
        ...
        Max = 40,
        Rounding = 0,
        Callback = function(v) Network.setLimit("DoRoll", v, v) end,
    })
    NetBox:AddSlider("SellRate", {
        ...
        Max = 20,
        Rounding = 0,
        Callback = function(v) Network.setLimit("SellCrate", v, v) end,
    })
            ...
            pcall(function()
                statsLabel:SetText(
                    string.format(
                        ...
                        ping,
                        netStats.totalFired + netStats.totalInvoked,
                        netStats.totalErrors
                    )
                )
